...
class DatabaseHelper {
  ...
  Future<bool> login(Users user) async {
    ...
    var result = await db
        .rawQuery("select * from users where usrName = '${user.usrName}'");
    if (result.isNotEmpty) {
      // Extract the hashed password from the database
      String storedHashedPassword = result[0]['usrPassword'].toString();
      // Compare the provided password with the stored hashed password
      if (BCrypt.checkpw(user.usrPassword, storedHashedPassword)) {
        return true;
      } else {
        return false;
      }
  ...
  Future<int> signup(Users user) async {
    ...
    final hashedPassword = BCrypt.hashpw(user.usrPassword, BCrypt.gensalt());
